...
class STSCBenchmark extends AnyFunSuite {
  ...
  test("Should work with 2 clusters of 100 observations in 2 dimensions") {
    val sample1Vector = MultivariateGaussian(DenseVector(0.0, 0.0), DenseMatrix((1.0, 0.0), (0.0, 1.0))).sample(100)
    val sample2Vector = MultivariateGaussian(DenseVector(0.0, 6.0), DenseMatrix((1.0, 0.0), (0.0, 1.0))).sample(100)

    val sample1 = DenseMatrix.zeros[Double](100, 2)
    val sample2 = DenseMatrix.zeros[Double](100, 2)

    for (i <- sample1Vector.indices) {
